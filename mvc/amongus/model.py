...
class AmongUs:
    ...
    async def start(self, ctx, members_list):
        await self.ctx.author.voice.channel.set_permissions(
            self.ctx.guild.default_role, use_voice_activation=True
        )
        ...
        amongstart.add_field(
            name="Players: ", value=", \n".join(members_list), inline=False
        )
        amongstart.add_field(
            name="Instructions ",
            value="All players in the voice channel can react to emoji to start the game.",
            inline=False,
        )
        ...
        def check(reaction, user):
            return user in self.ctx.author.voice.channel.members and (
                str(reaction.emoji) == "\U00002705"
                or str(reaction.emoji) == "\U0000274C"
            )
    ...
    async def playing(self, ctx):
        await self.ctx.author.voice.channel.set_permissions(
            ctx.guild.default_role, use_voice_activation=False
        )
        ...
        playing_embed.set_footer(
            text=(
                f"All players in voice channel can react to emoji to go to discussions round."
            )
        )
        ...
        def check(reaction, user):
            return user in self.ctx.author.voice.channel.members and (
                str(reaction.emoji) == "\U0001F508"
                or str(reaction.emoji) == "\U0001F503"
                or str(reaction.emoji) == "\U0000274C"
            )
    ...
    async def discussions(self, ctx):
        await self.ctx.author.voice.channel.set_permissions(
            ctx.guild.default_role, use_voice_activation=True
        )
        ...
        discussions_embed.set_footer(
            text=(
                f"All players in voice channel can react to emoji to go to playing round."
            )
        )
